...
time.
The pre-activation~$\vect{a}{t}$ at each timestep~$t$ is constructed from
both the ``hidden state'' from the previous timestep~$\vect{h}{t - 1}$ as well
as the input~$\vect{x}{t}$ of the current timestep, as given by
