...
network architectures~\citep{zoph2016neural}, gradient
descent optimizers~\citep{bello2017neural} and activation
functions~\citep{ramachandran2017searching}. Other options for architecture
search include evolutionary optimization~\citep{liu2018hierarchical}, Bayesian
optimization~\citep{malkomes2016bayesian}, and gradient descent-based methods.
...
extracted from the
image~\citep{fukui2016multimodalCB, Kim2017, ben2017mutan}.  The feature
...
of~\citet{Kim2017} and~\citet{ben2017mutan}, a pre-trained Residual Network
model~\citep{he2016deep} is used as a feature extractor for the image data
stream, and a pre-trained Skip-Thought
Vectors~\citep{kiros2015skip} model is used to extract features from the
...
The methods of~\citet{fukui2016multimodalCB},
\citet{Kim2017} and~\citet{ben2017mutan} all propose that in the multimodal
...
of~\citet{fukui2016multimodalCB} uses the Compact Bilinear Pooling
method of~\citet{gao2016compact} to approximate a bilinear interaction between
$\q$ and $\v$. In the case of MCB, the fusion operator~$\fusop$ consists of
projecting~$\q$ and~$\v$ using Count
Sketch~\citep{Charikar:2002:FFI:646255.684566}, followed by convolution of~$\q$
...
variables as described in~\citet{fukui2016multimodalCB}, is equal in
...
compute,~\citet{Kim2017} proposes a fusion operator based on the Hadamard
(element-wise) product.

The fusion operator in~\citet{Kim2017}, the Multimodal Low-rank Bilinear
...
both models under consideration~\citep{goyal2017making}. Therefore, we evaluate
...
\caption{A comparison between the MUTAN fusion operator of~\citet{ben2017mutan}
...
Following the work of~\citet{Kim2017} and~\citet{ben2017mutan}, we represent the
...
\end{table*}
\footnotetext[1]{\citet{fukui2016multimodalCB} as reported in~\citet{goyal2017making}}
\footnotetext[2]{\citet{ben2017mutan} as trained and evaluated by us}
\footnotetext[3]{ResNet features~$7 \times 7$ (single)~\citep{teney2017tips}}
\footnotetext[4]{Bottom-up attention image features, adaptive~$K$ (single)~\citep{teney2017tips}}
